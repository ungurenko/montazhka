...
struct ShortsDebugReplayTests {
    ...
    func testRealDeepSeekWrapperResponseUnwrapsWithoutRepair() throws {
        ...
        #expect((envelope.clips.map(\.id)) == (["confirm-plan-and-build", "agent-vs-chatbot"]))
        #expect((envelope.clips.first?.firstWordID) == ("w002593"))
    }
    ...
    func testRealDeepSeekWrapperWithoutFenceUnwraps() throws {
        let raw = """
            ...
            """
        let words = [
            OpenRouterTranscriptWord(id: "w002746", text: "а", start: 0, end: 1),
            OpenRouterTranscriptWord(id: "w002795", text: "б", start: 1, end: 2),
        ]
        let envelope = try OpenRouterClient.decodeShortsProposals(raw, words: words)
        #expect((envelope.clips.map(\.id)) == (["agent-vs-chatbot"]))
    }
}
