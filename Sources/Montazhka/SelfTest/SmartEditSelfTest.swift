...
enum SmartEditSelfTest {
    static func run() -> Int {
        ...
        func check(_ condition: Bool, _ label: String) {
            ...
            else { failures += 1; print("  ✗ ПРОВАЛ: \(label)") }
        }
        ...
        check(map.words.map(\.wordID) == ["w000001", "w000002", "w000003"] &&
              abs((map.words.first?.timelineStart ?? 0) - 1.1) < 0.001,
              "слова получают обезличенные ID и точные координаты ленты")
        check(map.range(firstWordID: "w000003", lastWordID: "w000001") == nil &&
        ...
        check(boundary != nil && (boundary!.timelineEnd - boundary!.timelineStart) >= 0.25,
        ...
        check(SmartEditSelection.shouldEnable(kind: .falseStart, confidence: 0.94, hasSafeBoundary: true) &&
        ...
        editor.apply(.replaceClips(TimelineOps.removingRange(
        ...
        check(editedCount == 2 && restored?.clips == [clip] && repeated?.clips.count == 2,
