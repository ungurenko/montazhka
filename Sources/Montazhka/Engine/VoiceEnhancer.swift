...
/// Ручной режим рендера выполняется вне вызывающего актора и не блокирует UI.
enum VoiceEnhancer {
    @concurrent
    static func render(
        sourcePath: String,
        ...
        try applyGain(
            from: tmpURL, to: outURL, gain: gain,
    ...
    private static func applyGain(
        from inURL: URL, to outURL: URL, gain: Float,
        sampleRate: Double, channels: AVAudioChannelCount,
        isCancelled: () -> Bool
    ) throws {
        let inFile = try AVAudioFile(
            forReading: inURL,
            commonFormat: .pcmFormatFloat32, interleaved: false)
