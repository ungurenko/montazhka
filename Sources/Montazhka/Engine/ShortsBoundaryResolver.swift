...
enum ShortsBoundaryResolver {
    ...
    static func resolve(first: MappedTranscriptWord,
                        last: MappedTranscriptWord,
                        peaks: [Float],
                        sourceDuration: Double,
                        thresholdDB: Double) -> Boundary? {
        ...
        let end = resolveEnd(lastWord: last, peaks: peaks, sourceDuration: sourceDuration,
                             thresholdDB: thresholdDB)
        guard end > start, end - start >= 0.5 else { return nil }
        return Boundary(start: start, end: end)
    }

    private static func resolveStart(firstWord: MappedTranscriptWord,
                                     peaks: [Float], thresholdDB: Double) -> Double {
        ...
        let quiet = quietPoint(near: target,
    ...
    private static func resolveEnd(lastWord: MappedTranscriptWord,
                                   peaks: [Float], sourceDuration: Double,
                                   thresholdDB: Double) -> Double {
    ...
    private static func quietPoint(near target: Double, peaks: [Float],
                                   thresholdDB: Double, lower: Double, upper: Double,
                                   preferLatest: Bool) -> Double? {
        ...
        return SmartCutBoundaryResolver.quietPoint(near: target, in: peaks, threshold: threshold,
